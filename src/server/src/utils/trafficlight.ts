...
export function Use (...middlewares: any[]): any {
  return (target: any, propertyKey: string, descriptor: TypedPropertyDescriptor<any>) => {
    propertyKey = !propertyKey ? '' : '_' + propertyKey
...
export function File () {
  return Inject((ctx) => {
    ...
    // }
    return ctx.request.body.files.file
  })
}
...
export function Files () {
  return Inject((ctx) => ctx.request.body.files)
}
...
/**
 * Current user decorator. This is a
 * shortcut for `ctx.state.$currentUser`.
 *
 * Example:
 *
 *    @Controller()
...
export function CurrentUser () {
  return Inject((ctx) => ctx.state.$currentUser)
}
